...
--
	...
	--Not generating user file.. Because of retaining the working directory setting that is user specified setting.
	--refer http://mediawiki.blueside.co.kr/index.php5/Premake-structure-notgenerateproject
	----------------------------------------------------------------------------
	function get_project_uuid(prj)
		...
		fh=io.open( fname , "r")  --projects.lua를 열어서
		if fh ~= nil then
			while true do  --라인별로 끝까지 반복한다
				line = fh.read(fh)
				if not line then break end  --라인끝이면 중단
				selection = string.match(line, "ProjectGuid")  --해당 keyword로 시작하는 부분이 있다면
				if selection ~=nil then  
					for w in string.gmatch(line, "[%w]+-[%w]+-[%w]+-[%w]+-[%w]+" ) do  -- UUID 만 얻어온다
						return w
					end
				end
			end
...
function vstudio.buildconfigs2(sln)
		local cfgs = { }
		...
		for _, buildcfg in ipairs(sln.configurations) do
			for _, platform in ipairs(platforms) do
				...
				--Blueside Customizing
				--for PS3 processing
				--refer http://mediawiki.blueside.co.kr/index.php5/Premake-structure-PS3_keyword
				----------------------------------------------------------------------------
				--if platform ~= "PS3" then
					entry.buildcfg = buildcfg
					entry.platform = vstudio.platforms[platform]
				--else
					--if platform == "PS3"  then --바꾸어서 host 표시되게
	...
	newaction 
	{
		...
		onproject = function(prj)
			...
			----------------------------------------------------------------------------
				...
				if prj.generateproject ~= "false" or getuuid ==-1 then
					premake.generate(prj, "%%.csproj", vstudio.cs2005.generate)
				end
			
			--	premake.generate(prj, "%%.csproj.user", vstudio.cs2005.generate_user)  --황제현 주석처리
			else
				...
				--premake.generate(prj, "%%.vcxproj.user", premake.vs2010_vcxproj_user)  --황제현 주석처리
				---------------------------------------------------------------------------
				--/End of Blueside Customizing
				----------------------------------------------------------------------------	
