...
--	
 	...
 	function premake.getlinks(cfg, kind, part, getprj)
		...
		for _, link in ipairs(cfg.links) do
			...
			elseif not prj and (kind == "system" or kind == "all") then
				...
				----------------------------------------------------------------------------
				elseif (part == "fullpath_project") then
					item = link
					if namestyle == "windows" then
						if premake.iscppproject(cfg) then
							item = item .. ".lib"
					...
					if item:find("/", nil, true) then
						item = path.getrelative(getprj.location, item)
					end
				---------------------------------------------------------------------------
...
--
	...
	function premake.gettarget(cfg, direction, pathstyle, namestyle, system)
		...
		elseif namestyle == "posix" then
			...
			---------------------------------------------------------------------------
			--Blueside Customizing
			--for PS3에서 objdir 및 library 생성파일에 앞에 lib 가 붙는현상때문에, 해당 prefix를 제거한다
			--refer http://mediawiki.blueside.co.kr/index.php5?title=Premake-structure-Delete-prefix-lib&action=edit&redlink=1
			----------------------------------------------------------------------------
				--prefix = "lib"    --Blueside Customizing
				prefix = ""
