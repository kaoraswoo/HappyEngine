...
--
	...
	function path.getrelative(src, dst)
		...
		-- Change Upper Character for Drive Name(only for getting relativepath)
		---------------------------------------------------------------------------
		local middle = string.match( src, "%l:" )
		if middle ~=nil then
			middle = middle:upper()
			src = string.gsub(src, "%l:", middle)
		end
		middle = string.match( dst, "%l:" )
		if middle ~=nil then
			middle = middle:upper()
			dst = string.gsub(dst, "%l:", middle)
		end
		---------------------------------------------------------------------------
